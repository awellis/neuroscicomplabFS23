...
Zuerst laden wir das `tidyverse` Package und lesen das im Ordner `data_example` gespeicherte `.csv` File ein und machen die Variable `condition` zu einem Faktor. Wir schauen uns die Daten mit `glimpse()` an.
